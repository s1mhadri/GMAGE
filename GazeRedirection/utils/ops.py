# This script contains all neural network layers and functions used in this project.
...
def conv2d(
    ...
    with tf.variable_scope(scope):
        ...
        if padding == "zero":
            ...
            x = tf.pad(input_, [[0, 0], [pad, pad], [pad, pad], [0, 0]], mode="REFLECT")
        else:
            x = input_
...
def style_loss(endpoints_mixed, style_layers):
    ...
    for layer in style_layers:
        feat_a, feat_b = tf.split(endpoints_mixed[layer], 2, 0)
        size = tf.size(feat_a)
        loss += tf.nn.l2_loss(gram(feat_a) - gram(feat_b)) * 2 / tf.to_float(size)
...
def angular_error(x, y):
    ...
    sim = np.divide(np.sum(np.multiply(x, y), axis=1), np.multiply(x_norm, y_norm))
