...
class Model(object):
    ...
    def __init__(self, params):
        ...
        self.x_valid_g = generator(self.x_valid_r, self.angles_valid_g, reuse=True)
    ...
    def data_loader(self):
        ...
        image_data_class = ImageData(
            ...
            ids=hps.ids,
            fold_size=hps.fold_size,
            n_people=hps.n_people,
            mode=hps.mode,
        )
        ...
        train_dataset = train_dataset.apply(
            shuffle_and_repeat(train_dataset_num)
        ).apply(
            map_and_batch(
        ...
        valid_dataset = test_dataset.apply(shuffle_and_repeat(test_dataset_num)).apply(
            map_and_batch(
                image_data_class.image_processing,
                hps.batch_size,
                num_parallel_batches=8,
    ...
    def adv_loss(self):
        ...
        g_loss: scalar, adcersarial loss for training generator.
        ...
        eps = tf.random_uniform(shape=[hps.batch_size, 1, 1, 1], minval=0.0, maxval=1.0)
        interpolated = eps * self.x_r + (1.0 - eps) * self.x_g
        ...
        d_loss = -tf.reduce_mean(gan_real) + tf.reduce_mean(gan_fake) + 10.0 * gp
    ...
    def feat_loss(self):
        ...
        _, endpoints_mixed = vgg_16(tf.concat([self.x_g, self.x_t], 0))
    ...
    def optimizer(self, lr):
        ...
        if hps.optimizer == "adam":
            return tf.train.AdamOptimizer(
                lr, beta1=hps.adam_beta1, beta2=hps.adam_beta2
            )
    ...
    def add_optimizer(self):
        ...
        g_op = g_opt.minimize(
            loss=g_loss, global_step=self.global_step, var_list=g_vars
        )
        d_op = d_opt.minimize(
            loss=d_loss, global_step=self.global_step, var_list=d_vars
        )
    ...
    def add_summary(self):
        ...
        tf.summary.image(
            "fake", tf.clip_by_value((self.x_g + 1) / 2.0, 0.0, 1.0), max_outputs=5
        )
        tf.summary.image(
            "recon", tf.clip_by_value((self.x_recon + 1) / 2.0, 0.0, 1.0), max_outputs=5
        )

        tf.summary.image(
            "x_test",
            tf.clip_by_value((self.x_valid_r + 1) / 2.0, 0.0, 1.0),
            max_outputs=5,
        )
        tf.summary.image(
            "x_test_fake",
            tf.clip_by_value((self.x_valid_g + 1) / 2.0, 0.0, 1.0),
            max_outputs=5,
        )
                    ...
                    for it in range(num_iter):
                        ...
                        if it % hps.summary_steps == 0:
                            ...
                            summary, global_step = sess.run(
                                [self.summary, self.global_step], feed_dict=feed_d
                            )
    ...
    def eval(self):
        ...
        dir_name = hps.data_path.split("/")[-2]
        ...
        with tf.Session(config=tf_config) as test_sess:
            ...
            with test_sess.graph.as_default():
                ...
                imgs_dir = os.path.join(hps.log_dir, os.pardir, "eval")
                if not os.path.exists(imgs_dir):
                    os.mkdir(imgs_dir)

                # gene_dir = os.path.join(imgs_dir, "Test" + dir_name)
                gene_dir = os.path.join(imgs_dir, "mpii2col")
                if not os.path.exists(gene_dir):
                    os.makedirs(gene_dir)

                try:
                    i = 0  # sequence number to prevent duplicates overwriting each other
                    while True:
                        (real_imgs, keys, fake_imgs, a_r, a_t) = test_sess.run(
                            [
                                self.x_test_r,
                                self.labels_test,
                                x_fake,
                        ...
                        ]  # convert literal byte to string
                        idxs = [
                            int(key.split("_")[0]) for key in keys
                        ]  # particpant number
                        head_poses = [key.split("_")[1] for key in keys]
                        sides = [key.split("_")[2] for key in keys]

                        for j in range(real_imgs.shape[0]):
                            imwrite(
                                os.path.join(
                                    gene_dir,
                                    "%04d_%04d_%.2f_%s_H%.2f_V%.2f_%s.jpg"
                                    % (
                                        idxs[j],
                                        i,
                                        delta[j],
