...
class ImageData(object):
    ...
    def preprocess(self):
        ...
        # )
        val_ids = list(range(0, 16, 3))
        train_ids = [j+1 for j in range(self.n_people) if j+1 not in val_ids]

        for key in self.file_dict.keys():
            idx = int(key.split("_")[0])  # particpant number
            eye = key.split('_')[-1]
            headp = float(key.split('_')[-2].strip("P"))
            flip = 1
            if eye == "R":
                flip = -1
            
            if (headp == 0 and eye == 'R'):
                for f_r in np.random.choice(self.file_dict[key], 200, replace=False):
                    ...
                    for f_g in np.random.choice(self.file_dict[key], 150, replace=False):
                        ...
                        cond = h_angle_r != h_angle_g or v_angle_r != v_angle_g
