...
class ImageData(object):
    ...
    def preprocess(self):
        ...
        train_ids = [j+1 for j in range(self.n_people) if j+1 not in val_ids]
        n = 5
        ...
        for key in self.file_dict.keys():
            ...
            for f_r in self.file_dict[key]:
                ...
                h_ang = flip * float(f_r.split("_")[-2].strip("H"))
                v_ang = float(f_r.split("_")[-3].strip("V"))
                ...
                # hlist = [np.random.randint(-25, -4), np.random.randint(5, 26)]
                # vlist = [np.random.randint(-15, -4), np.random.randint(5, 16)]
                # h_angle_g = np.random.choice(hlist, 1)[0] / 15
                # v_angle_g = np.random.choice(vlist, 1)[0] / 10
                # cond = -2 < h_ang < 2 and -2 < v_ang < 2

                h_angle_g = 0.0
                v_angle_g = 0.0
                cond = (-n > h_ang or h_ang > n) and (-n > v_ang or v_ang > n)

                if self.mode == "train" and cond:
                    if idx in train_ids:
