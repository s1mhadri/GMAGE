...
class ELG(BaseModel):
    ...
    def train_loop_pre(self, current_step):
        ...
        data_source.set_difficulty(min((1.0 / 1e6) * current_step, 1.0))
        ...
        with tf.variable_scope("input_data"):
            ...
            if y1 is not None:
                self.summary.feature_maps(
                    "hmaps_true", y1, data_format=self._data_format_longer
                )
        ...
        with tf.variable_scope("hourglass"):
            ...
            for i in range(self._hg_num_modules):
                with tf.variable_scope("hg_%d" % (i + 1)):
                    ...
                    self.summary.feature_maps(
                        "hmap%d" % i, h, data_format=self._data_format_longer
                    )
                    if y1 is not None:
    ...
    def _build_hourglass_after(self, x_prev, x_now, do_merge=True):
        with tf.variable_scope("after"):
            ...
            with tf.variable_scope("hmap"):
                h = self._apply_conv(
                    x_now, self._hg_num_landmarks, kernel_size=1, stride=1
                )
            ...
            with tf.variable_scope("merge"):
                with tf.variable_scope("h"):
                    x_hmaps = self._apply_conv(
                        h, self._hg_num_feature_maps, kernel_size=1, stride=1
                    )
...
def estimate_gaze_from_landmarks(
    iris_landmarks, iris_centre, eyeball_centre, eyeball_radius, initial_gaze=None
):
    ...
    if initial_gaze is not None:
        ...
        phi = np.arcsin(
            np.clip((i_x0 - e_x0) / (eyeball_radius * -np.cos(theta)), -1.0, 1.0)
        )
    ...
    def gaze_fit_loss_func(inputs):
        ...
        dx1_ddelta = (
            -cos_theta * dcos_delta_sin_ddelta + sin_theta * dsin_delta_sin_ddelta
        )
        dx2_ddelta = sin_phi * dcos_delta_cos_ddelta + cos_phi * dsin_delta_cos_ddelta
        dx1_dphase = (
            -cos_theta * dcos_delta_sin_dphase + sin_theta * dsin_delta_sin_dphase
        )
        ...
        # out += iris_dx ** 2 + iris_dy ** 2
        dout_dtheta += (
            2 * eyeball_radius * (sin_theta * sin_phi * iris_dx + cos_theta * iris_dy)
        )
