...
class DPG(BaseModel):
        ...
        with tf.variable_scope("augment"):
            ...
            transforms = tf.concat(
                [
                    ...
                    tf.truncated_normal((n, 1), mean=0, stddev=0.05 * w),
                    self._column_of_zeros,
                    self._column_of_ones,
                    tf.truncated_normal((n, 1), mean=0, stddev=0.05 * h),
                    self._column_of_zeros,
                    self._column_of_zeros,
                ],
                axis=1,
            ...
            images = tf.contrib.image.transform(
                images, transforms, interpolation="BILINEAR"
            )
            if self._data_format == "NCHW":
    ...
    def build_model(self, data_sources: Dict[str, BaseDataSource], mode: str):
        ...
        with tf.variable_scope("input_data"):
            ...
            if y1 is not None:
                self.summary.feature_maps(
                    "gazemaps", y1, data_format=self._data_format_longer
                )
        ...
        with tf.variable_scope("hourglass"):
            ...
            with tf.variable_scope("pre"):
                ...
                x = self._apply_conv(
                    x, num_features=n, kernel_size=7, stride=self._hg_first_layer_stride
                )
            ...
            for i in range(self._hg_num_modules):
                with tf.variable_scope("hg_%d" % (i + 1)):
                    x = self._build_hourglass(
                        x, steps_to_go=4, num_features=self._hg_num_feature_maps
                    )
            ...
            if y1 is not None:
                # Cross-entropy loss
                metrics["gazemaps_ce"] = -tf.reduce_mean(
                    tf.reduce_sum(
            ...
            self.summary.feature_maps(
                "bottleneck", gmap, data_format=self._data_format_longer
            )
        ...
        with tf.variable_scope("densenet"):
            ...
            for i in range(self._dn_num_dense_blocks):
                with tf.variable_scope("block%d" % (i + 1)):
                    x = self._apply_dense_block(
                        x, num_layers=self._dn_num_layers_per_block[i]
                    )
            ...
            with tf.variable_scope("output"):
                ...
                if y2 is not None:
                    ...
                    metrics[
                        "gaze_ang"
                    ] = util.gaze.tensorflow_angular_error_from_pitchyaw(y2, x)

        # Combine two loss terms
        if y1 is not None and y2 is not None:
            loss_terms["combined_loss"] = (
                1e-5 * metrics["gazemaps_ce"] + metrics["gaze_mse"]
            )
    ...
    def _build_residual_block(self, x, num_in, num_out, name="res_block"):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope("conv1"):
                ...
                c = self._apply_conv(
                    c, num_features=half_num_out, kernel_size=1, stride=1
                )
            with tf.variable_scope("conv2"):
                c = tf.nn.relu(self._apply_bn(c))
                c = self._apply_conv(
                    c, num_features=half_num_out, kernel_size=3, stride=1
                )
            ...
            with tf.variable_scope("skip"):
                if num_in == num_out:
                    ...
                    s = self._apply_conv(
                        x, num_features=num_out, kernel_size=1, stride=1
                    )
            x = c + s
        return x
    ...
    def _build_hourglass(self, x, steps_to_go, num_features, depth=1):
        with tf.variable_scope("depth%d" % depth):
            ...
            for i in range(self._hg_num_residual_blocks):
                up1 = self._build_residual_block(
                    up1, num_features, num_features, name="up1_%d" % (i + 1)
                )
            # Lower branch
            low1 = self._apply_pool(x, kernel_size=2, stride=2)
            for i in range(self._hg_num_residual_blocks):
                low1 = self._build_residual_block(
                    low1, num_features, num_features, name="low1_%d" % (i + 1)
                )
            # Recursive
            low2 = None
            if steps_to_go > 1:
                low2 = self._build_hourglass(
                    low1, steps_to_go - 1, num_features, depth=depth + 1
                )
            else:
                low2 = low1
                for i in range(self._hg_num_residual_blocks):
                    low2 = self._build_residual_block(
                        low2, num_features, num_features, name="low2_%d" % (i + 1)
                    )
            # Additional residual blocks
            low3 = low2
            for i in range(self._hg_num_residual_blocks):
                low3 = self._build_residual_block(
                    low3, num_features, num_features, name="low3_%d" % (i + 1)
                )
            # Upsample
    ...
    def _build_hourglass_after(self, x_prev, x_now, do_merge=True):
        with tf.variable_scope("after"):
            ...
            x_now = self._apply_conv(
                x_now, self._hg_num_feature_maps, kernel_size=1, stride=1
            )
            x_now = self._apply_bn(x_now)
            x_now = tf.nn.relu(x_now)

            with tf.variable_scope("gmap"):
                gmap = self._apply_conv(
                    x_now, self._hg_num_gazemaps, kernel_size=1, stride=1
                )
        ...
        if do_merge:
            with tf.variable_scope("merge"):
                with tf.variable_scope("gmap"):
                    x_gmaps = self._apply_conv(
                        gmap, self._hg_num_feature_maps, kernel_size=1, stride=1
                    )
            ...
            with tf.variable_scope("layer%d" % (i + 1)):
                ...
                with tf.variable_scope("bottleneck"):
                    x = self._apply_composite_function(
                        x, num_features=min(n, 4 * self._dn_growth_rate), kernel_size=1
                    )
                with tf.variable_scope("composite"):
                    x = self._apply_composite_function(
                        x, num_features=self._dn_growth_rate, kernel_size=3
                    )
                if self._data_format == "NCHW":
    ...
    def _apply_transition_layer(self, x):
        ...
        x = self._apply_composite_function(
            x,
            num_features=int(self._dn_compression_factor * x.shape.as_list()[c_index]),
            kernel_size=1,
        )
    ...
    def _apply_composite_function(self, x, num_features=_dn_growth_rate, kernel_size=3):
        ...
        x = tf.nn.relu(x)
        x = self._apply_conv(
            x, num_features=num_features, kernel_size=kernel_size, stride=1
        )
        return x
