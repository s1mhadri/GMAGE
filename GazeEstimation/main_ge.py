...
if __name__ == "__main__":
    ...
    for fold_id in range(1):
        ...
        # train_range = np.random.choice(45, 45-fold_id, replace=False)
        train_range = range(fold_id, 45)
        val_range = range(45, 56)
        # val_ids = sorted(["%04d" % (j + 1) for j in val_range])
        # train_ids = ["%04d" % (j + 1) for j in range(n_people) if j not in list(val_range)]
        # train_ids = ["%04d" % (j + 1) for j in range(n_people)]
        # val_ids = ["%04d" % (2*j + 1) for j in range(n_people)]
        val_ids = sorted(["%04d" % (j + 1) for j in val_range])
