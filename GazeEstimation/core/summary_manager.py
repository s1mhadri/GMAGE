...
class SummaryManager(object):
    ...
    def _prepare_for_write(self):
        ...
        self._writer = tf.summary.FileWriter(
            self._model.output_path, self._tensorflow_session.graph
        )
    ...
    def get_ops(self, mode="train"):
        ...
        if mode == "test" or mode == "full_test":  # Always return all ops for test case
            ...
            if check_func(
                "expensive_summaries_train", self._expensive_ops_every_n_secs
            ):
                return self._expensive_ops[mode]
    ...
    def _register_cheap_op(self, operation):
        ...
        try:
            assert (
                name not in self._cheap_ops[mode]
                and name not in self._expensive_ops[mode]
            )
    ...
    def filters(self, name, tensor, **kwargs):
        ...
        with tf.name_scope("viz_filters"):
            ...
            matching_tensors = [
                t for t in tf.trainable_variables() if t.name.startswith(name_stem)
            ]
