...
class BaseDataSource(object):
    ...
    def __init__(
        ...
        assert tensorflow_session is not None and isinstance(
            tensorflow_session, tf.Session
        )
        ...
        with tf.variable_scope("".join(c for c in self.short_name if c.isalnum())):
            ...
            self._preprocess_queue_capacity = (
                min_after_dequeue + (num_threads + 1) * batch_size
            ...
            self._enqueue_op = self._preprocess_queue.enqueue(
                tuple(self._tensors_to_enqueue.values())
            )
            self._preprocess_queue_close_op = self._preprocess_queue.close(
                cancel_pending_enqueues=True
            )
            self._preprocess_queue_size_op = self._preprocess_queue.size()
            self._preprocess_queue_clear_op = self._preprocess_queue.dequeue_up_to(
                self._preprocess_queue.size()
            )
            if not staging:
                ...
                self._staging_area_put_op = self._staging_area.put(
                    self._preprocess_queue.dequeue_many(batch_size)
                )
                ...
                self._output_tensors = dict(
                    [
                        (label, tensor)
                        for label, tensor in zip(labels, self._staging_area.get())
                    ]
                )
    ...
    def cleanup(self):
        ...
        preprocess_threads = [
            t for t in self.all_threads if t.name.startswith("preprocess_")
        ]
        transfer_threads = [
            t for t in self.all_threads if t.name.startswith("transfer_")
        ]
    ...
    def reset(self):
        ...
        while True:  # Unblock any enqueue requests
            preprocess_queue_size = self._tensorflow_session.run(
                self._preprocess_queue_size_op
            )
    ...
    def preprocess_job(self):
        ...
        while not self._coordinator.should_stop():
            ...
            if preprocessed_entry_dict is not None:
                feed_dict = dict(
                    [
                        (self._tensors_to_enqueue[label], value)
    ...
    def create_threads(self):
        ...
        for i in range(self.num_threads):
            # File read thread
            _create_and_register_thread(
                target=self.read_entry_job, name="fread_%s_%d" % (name, i)
            )

            # Preprocess thread
            _create_and_register_thread(
                target=self.preprocess_job, name="preprocess_%s_%d" % (name, i)
            )

        if self.staging:
            # Send-to-GPU thread
            _create_and_register_thread(
                target=self.transfer_to_gpu_job, name="transfer_%s_%d" % (name, i)
            )
