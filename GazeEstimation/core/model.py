...
class BaseModel(object):
    ...
    def __init__(
        ...
        for data_source in all_data_sources:
            if data_source.batch_size != self._batch_size:
                raise ValueError(
                    (
                        'Data source "%s" has anomalous batch size of %d '
                        + "when detected batch size is %d."
                    )
                    % (data_source.short_name, data_source.batch_size, self._batch_size)
                )
            if data_source.data_format != self._data_format:
                raise ValueError(
                    (
                        'Data source "%s" has anomalous data_format of %s '
                        + "when detected data_format is %s."
                    )
                    % (
        ...
        self._data_format_longer = (
            "channels_first" if self._data_format == "NCHW" else "channels_last"
        )
        ...
        self._enable_live_testing = (len(self._train_data) > 0) and (
            len(self._test_data) > 0
        )
        ...
        with tf.variable_scope("learning_params"):
            ...
            self.learning_rate_multiplier = tf.Variable(
            ...
            self.assign_learning_rate_multiplier = tf.assign(
                self.learning_rate_multiplier, self.learning_rate_multiplier_placeholder
            )
    ...
    def identifier(self):
        ...
        if (
            self.__identifier is not None
        ):  # If loading from checkpoints or having naming enforced
            return self.__identifier
        if self.__identifier_stem is None:
            self.__identifier_stem = (
    ...
    def output_path(self):
        ...
        return "%s/%s" % (
            os.path.abspath(os.path.dirname(__file__) + "/../../%s" % (self._log_dir)),
            self.identifier,
        )
    ...
    def _build_all_models(self):
                    ...
                    for key, tensor in tensors.items():
                        ...
                        if num_dims == 4:  # Image data
                            if shape[1] == 1 or shape[1] == 3:
                                self.summary.image(
                                    summary_name, tensor, data_format="channels_first"
                                )
                            elif shape[3] == 1 or shape[3] == 3:
                                self.summary.image(
                                    summary_name, tensor, data_format="channels_last"
                                )
                            ...
                            logger.debug(
                                "I do not know how to create a summary for %s (%s)"
        ...
        def _build_train_or_test(mode):
            ...
            # Build model
            output_tensors, loss_terms, metrics = self.build_model(
                data_sources, mode=mode
            )
        ...
        if len(self._train_data) > 0:
            ...
            logger.info("------------------------------")
            logger.info(
                "FLOPS per input: {:,}".format(flops.total_float_ops / self._batch_size)
            )
            logger.info(
        ...
        for spec in self._learning_schedule:
            ...
            for loss_term_key, prefixes in loss_terms.items():
                ...
                for prefix in prefixes:
                    variables_to_train += [
                        v for v in all_trainable_variables if v.name.startswith(prefix)
                    ]
                    update_ops += [
                        o for o in all_update_ops if o.name.startswith(prefix)
                    ]
                    reg_losses += [
                        l for l in all_reg_losses if l.name.startswith(prefix)
                    ]
    ...
    def train(self, num_epochs=None, num_steps=None):
        ...
        if num_steps is None:
            num_entries = np.min(
                [s.num_entries for s in list(self._train_data.values())]
            )
        ...
        try:
            ...
            for current_step in range(initial_step, num_steps):
                ...
                loss_term_keys, _ = zip(
                    *list(schedule["loss_terms_to_optimize"].items())
                )
                fetches["loss_terms"] = [
                    self.loss_terms["train"][k] for k in loss_term_keys
                ]
                ...
                # Save model weights
                if (
                    self.time.has_been_n_seconds_since_last("save_weights", 600)
