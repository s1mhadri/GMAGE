...
class LiveTester(object):
    ...
    def do_full_test(self, sleep_between_batches=0.2):
        ...
        feed_dict = dict(
            [(self._placeholders[name], value) for name, value in outputs.items()]
        )
    ...
    def _post_model_build(self):
        ...
        with tf.variable_scope("copy2test"):
            ...
            train_vars = dict(
                [(v.name, v) for v in all_variables if not v.name.startswith("test/")]
            )
            test_vars = dict(
                [(v.name, v) for v in all_variables if v.name.startswith("test/")]
            )
            self._copy_variables_to_test_model_op = tf.tuple(
                [
                    test_vars["test/" + k].assign(train_vars[k])
                    for k in train_vars.keys()
        ...
        self._thread = threading.Thread(
            target=self.test_job, name="%s_tester" % self.model.identifier
        )
        ...
        loss_terms_to_evaluate = dict(
            [
                (n, t)
                for n, t in self.model.loss_terms["test"].items()
        ...
        metrics_to_evaluate = dict(
            [
                (n, t)
                for n, t in self.model.metrics["test"].items()
                if t in self._tensors_to_evaluate.values()
            ]
        )
        ...
        self._placeholders = {}
        for type_, tensors in (
            ("loss", loss_terms_to_evaluate),
            ("metric", metrics_to_evaluate),
        ):
            for name in tensors.keys():
                name = "%s/test/%s" % (type_, name)
                placeholder = tf.placeholder(
                    dtype=np.float32, name=name + "_placeholder"
                )
