...
class FramesSource(BaseDataSource):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._frame_read_thread = threading.Thread(
            target=self.frame_read_job, name="frame_read"
        )
        ...
        while True:
            ...
            if bgr is not None:
                ...
                with self._read_mutex:
                    ...
                    self._frame_read_queue.put_nowait(
                        (before_frame_read, bgr, after_frame_read)
                    )
        self._open = False
    ...
    def detect_faces(self, frame):
        ...
        if (
            "last_face_detect_index" not in previous_frame
            ...
            if detector.__class__.__name__ == "CascadeClassifier":
                ...
                detections = detector(
                    cv.resize(frame["grey"], (0, 0), fx=0.5, fy=0.5), 0
                )
            faces = []
            for d in detections:
                try:
    ...
    def detect_landmarks(self, frame):
        ...
        for face in frame["faces"]:
            ...
            rectangle = dlib.rectangle(
                left=int(l), top=int(t), right=int(l + w), bottom=int(t + h)
            )
            ...
            landmarks.append(
                np.array([tuple_from_dlib_shape(i) for i in range(num_landmarks)])
            )
    ...
    def calculate_smoothed_landmarks(self, frame):
        ...
        if self._smoothing_coefficients is None:
            coefficients = np.power(
                self._smoothing_coefficient_decay,
        ...
        window_num_landmark_entries = np.array(
            [len(f["landmarks"]) for f in window_frames]
        )
        ...
        frame["smoothed_landmarks"] = np.sum(
            np.multiply(
                window_landmarks.reshape(self._smoothing_window_size, -1),
        ...
        for face, landmarks in zip(frame["faces"], frame_landmarks):
            ...
            for corner1, corner2, is_left in [(2, 3, True), (0, 1, False)]:
                ...
                eye_width = 1.5 * np.linalg.norm(
                    landmarks[corner1, :] - landmarks[corner2, :]
                )
                if eye_width == 0.0:
                    continue
                ...
                inv_transform_mat = (
                    inv_translate_mat * inv_rotate_mat * inv_scale_mat * inv_centre_mat
                )
...
def _get_opencv_xml(xml_name):
    ...
    if not os.path.isfile(xml_path):
        url_stem = (
            "https://raw.githubusercontent.com/opencv/opencv/master/data/lbpcascades"
        )
