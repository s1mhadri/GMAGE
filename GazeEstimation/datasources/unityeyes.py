...
class UnityEyes(BaseDataSource):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._file_stems = sorted(
            [p[:-5] for p in os.listdir(unityeyes_path) if p.endswith(".json")]
        )
        ...
        super().__init__(
            tensorflow_session, batch_size=batch_size, testing=testing, **kwargs
        )
    ...
    def preprocess_entry(self, entry):
        ...
        def value_from_type(augmentation_type):
            ...
            value = (
                np.clip(value, easy_value, hard_value)
        ...
        scale = 1.0 + noisy_value_from_type("scale")
        scale_inv = 1.0 / scale
        np.fill_diagonal(scale_mat, ow / eye_width * scale)
        original_eyeball_radius = 71.7593
        eyeball_radius = (
            original_eyeball_radius * scale_mat[0, 0]
        )  # See: https://goo.gl/ZnXgDE
        ...
        recentre_mat[1, 2] = (
            ih / 2 - eye_middle[1] + 0.5 * oh / ow * eye_width * scale_inv
        )
        ...
        iris_centre = np.asarray(
            [
                iw_2
                + original_eyeball_radius
                * -np.cos(original_gaze[0])
